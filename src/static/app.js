      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          <ul class="participants-list">
            ${details.participants.length > 0 ? details.participants.map(p => `<li class="participant-item">${p} <button type="button" class="remove-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}" aria-label="Remove ${p}">✕</button></li>`).join('') : '<li class="participant-item">No participants yet.</li>'}
          </ul>
        `;
